...
def test_validator_hallucinated_citation_rejected():
    ...
    answer = "Vata is described here [cs_su_01_015], but another text mentions it elsewhere [cs_su_02_099]."
    result = validate_response(answer, retrieved)
...
def test_validator_classical_factual_dosage_allowed():
    ...
    answer = "The classical text records that the formulation was prepared with four palas of ghee [cs_su_01_015]."
    result = validate_response(answer, retrieved)
    assert result.is_valid is True
